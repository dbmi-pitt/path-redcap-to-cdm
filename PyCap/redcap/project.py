...
class Project(object):
    ...
    # added by CDB 3/21/2016
    def get_metadata(self, metadatakey=None, format='json', df_kwargs=None):
        ret_format = format
        if format == 'df':
        ...
        if format in ('json', 'csv', 'xml'):
            ...
            print "Format " + format + " unrecognized by get_metadata.  Returning empty string."
            return ''

    def get_version(self, format='json', df_kwargs=None):
        """
        ...
        if format == 'df':
            ...
            from pandas import read_csv
            ret_format = 'csv'
        pl = self.__basepl('versiond', format=ret_format)
        # to_add = [arms]
        # str_add = ['arms']
        # for key, data in zip(str_add, to_add):
        #     if data:
        #         pl[key] = ','.join(data)
        response, _ = self._call_api(pl, 'version')
        if format in ('json', 'csv', 'xml'):
            return response
        elif format == 'df':
            if not df_kwargs:
    ...
    def export_events(self, arms=None, format='json', df_kwargs=None):
        """
        Export the project's events
        ...
        Returns
        -------
        events : list, str, ``pandas.DataFrame``
        ...
        if format == 'df':
            ...
            from pandas import read_csv
            ret_format = 'csv'
        pl = self.__basepl('event', format=ret_format)
        to_add = [arms]
        str_add = ['arms']
        for key, data in zip(str_add, to_add):
            if data:
                pl[key] = ','.join(data)
        response, _ = self._call_api(pl, 'exp_event')
        if format in ('json', 'csv', 'xml'):
            return response
        elif format == 'df':
            if not df_kwargs:
        ...
        for key, data in zip(str_keys, keys_to_add):
            if data:
                ...
                if key in ('fields', 'records', 'forms', 'events'):
                    # remove any 'records' elements where record_id is None
                    while data.count(None) > 0:
                        print "key: " + key
    ...
    def backfill_fields(self, fields, forms):
        ...
        if forms and not fields:
            ...
            # added CDB 12/15/15
            # One of the def_field is set to None.
            # do not add this def_field because it causes problems
            # when the other parts of the code use the fields
            if (self.def_field != None):
